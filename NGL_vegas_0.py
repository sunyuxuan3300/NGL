...
def f(x):
    ...
    nm = 4*(-1+2*x[1])*np.cos(1*x[0])*(T1+T2+T3+T4)
...
def main():
    ...
    # 适应性迭代阶段
    print("\n=== 开始适应性迭代阶段 ===")
    training_result = integ(f, nitn=80, neval=1e6, analyzer=vegas.reporter())

    # 最终积分阶段
    print("\n=== 开始最终积分阶段 ===")
    result = integ(f, nitn=200, adapt=False, neval=2e7, analyzer=vegas.reporter())
