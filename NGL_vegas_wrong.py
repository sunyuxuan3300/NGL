...
def f(x):
    ...
    jacobian = (pi/2)**4 * np.sin(2*theta1) * np.sin(2*theta2) * \
               np.sin(2*theta3) * np.sin(2*theta6) / 4
    ...
    # T2项
    T2 = (2*(-2+x[4])*(-2+x[5])*np.log(x1) + 
          2*(-2+x[4])*(-2+x[5])*np.log(1-x1) + 
          8*np.log(1-x2) - 4*x[4]*np.log(1-x2) - 
          4*x[5]*np.log(1-x2) + 2*x[4]*x[5]*np.log(1-x2))
    
    # T3项
    T3 = (8*np.log(x2) - 4*x[4]*np.log(x2) - 4*x[5]*np.log(x2) + 
          2*x[4]*x[5]*np.log(x2) - 
    ...
    de = (np.pi**3 * (2-x3) * (-2+x[4]) * (-2+x[5]) * 
          (2-x3 + 2*(-1+2*x1)*np.sqrt(2-x3)*np.sqrt(x3)*np.sqrt(x6) + x3*x6))
...
def main():
    ...
    result = integ(f, nitn=200, adapt=False, neval=2e7, 
                  analyzer=vegas.reporter())
